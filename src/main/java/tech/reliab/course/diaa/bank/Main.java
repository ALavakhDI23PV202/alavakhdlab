	...
	public static void main(String[] args) {
		...
		for (Bank bank : banks) {
			bankOfficeService.create(new BankOffice(
					id, "Офис " + id + " банка " + bank.getName(), "Костюкова 46 " + id, bank, true,
		...
		for (BankOffice bankOffice : bankOffices) {
			atmService.create(new BankAtm(
					id, "Банкомат " + id + " офиса " + bankOffice.getName(), Status.WORKING, true,
					true, random.nextInt(1000) + 500, random.nextInt(100) + 100, bankOffice));

			id += 1;

			atmService.create(new BankAtm(
					id, "Банкомат " + id + " офиса " + bankOffice.getName(), Status.WORKING, false,
					true, random.nextInt(1000) + 500, random.nextInt(100) + 100, bankOffice));
